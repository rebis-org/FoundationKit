    ...
    // why: FSEventStreamCallback C ABI fixes the arity.
    @unsafe // swiftlint:disable:next function_parameter_count
    ...
    final class FSEvents: @unchecked Sendable {
        ...
        private func makeEvent(path: String, flags: FSEventStreamEventFlags, eventID: UInt64) -> FileSystemEvent {
        ...
        private func isExcluded(_ url: URL) -> Bool {
            ...
            for component in relativeComponents {
                for pattern in options.excludePatterns where matchesGlobPattern(name: String(component), pattern: pattern) {
                    return true
                }
            }
            return false
