...
public struct Probe: Sequence {
    ...
    init(root: Path, volume: any Volume, depth: ClosedRange<Int>, types: Set<EntryType>, extensions: Set<String>, hidden: Bool) {
        self.root = root
        self.volume = volume
        self.depth = depth
        self.types = types
    ...
    private func withDepth(_ depth: ClosedRange<Int>) -> Self {
        Self(root: root, volume: volume, depth: depth, types: types, extensions: extensions, hidden: hidden)
    }

    private func withTypes(_ types: Set<EntryType>) -> Self {
        Self(root: root, volume: volume, depth: depth, types: types, extensions: extensions, hidden: hidden)
    }

    private func withExtensions(_ extensions: Set<String>) -> Self {
        Self(root: root, volume: volume, depth: depth, types: types, extensions: extensions, hidden: hidden)
    }

    private func withHidden(_ hidden: Bool) -> Self {
        Self(root: root, volume: volume, depth: depth, types: types, extensions: extensions, hidden: hidden)
    }

    public func depth(max: Int) -> Self {
        withDepth(Swift.min(max, depth.lowerBound) ... max)
    }

    public func depth(min: Int) -> Self {
        withDepth(min ... Swift.max(depth.upperBound, min))
    }

    public func depth(_ range: Range<Int>) -> Self {
        withDepth(range.lowerBound ... (range.upperBound - 1))
    }
    ...
    public func type(_ type: EntryType) -> Self {
        withTypes(types.union([type]))
    }

    public func `extension`(_ fileExtension: String) -> Self {
        withExtensions(extensions.union([fileExtension]))
    }
...
public struct ProbeIterator: IteratorProtocol {
    ...
    public mutating func next() -> Path? {
        ...
        while let url = enumerator.nextObject() as? URL {
            ...
            if !probe.types.isEmpty, let type = probe.volume.type(at: path), !probe.types.contains(type) {
                continue
            }
