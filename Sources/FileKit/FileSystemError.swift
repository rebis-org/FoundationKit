...
public enum FileSystemError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .pathNotFound(path):
            failure("find path", path)

        case .emptyFilePath:
            failure("use empty file path")

        case .cannotRenameRoot:
            failure("rename file-system root")

        case let .renameFailed(path, error):
            failure("rename", path, because: error)

        case let .moveFailed(path, error):
            failure("move", path, because: error)

        case let .copyFailed(path, error):
            failure("copy", path, because: error)

        case let .deleteFailed(path, error):
            failure("delete", path, because: error)

        case let .unresolvedSearchPath(searchPath, domain):
            failure("resolve search path \(searchPath) in domain \(domain)")

        case .emptyWritePath:
            failure("write to empty path")

        case let .directoryCreationFailed(path, error):
            failure("create directory", path, because: error)

        case let .fileCreationFailed(path):
            failure("create file", path)

        case let .writeFailed(path, error):
            failure("write", path, because: error)

        case let .stringEncodingFailed(string):
            failure("encode string", string)

        case let .readFailed(path, error):
            failure("read", path, because: error)

        case let .stringDecodingFailed(path):
            failure("decode string at", path)

        case let .notAnInt(path, value):
            failure("parse integer from", path, because: "'\(value)'")

        case let .cannotOpenDirectory(url):
            failure("open directory", url.path)
        ...
        case let .failedToWatch(url, error):
            failure("watch", url.path, because: error)
        }
    }
