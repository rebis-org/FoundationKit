...
extension Item {
    ...
    public func makeDirectory(
        ...
        try volume.createDirectory(
            at: path, createIntermediateDirectories: options == .createIntermediateDirectories)
        return path
    }

    public func rename(to newName: String, using volume: some Volume = FoundationVolume()) throws
        -> Path {
    ...
    public func symlink(as destination: some Item, using volume: some Volume = FoundationVolume())
        throws -> Path {
        let destinationPath = Path(destination)
    ...
    public func symlink(into directory: some Item, using volume: some Volume = FoundationVolume())
        throws -> Path {
        let directoryPath = Path(directory)
    ...
    public func list(
        ...
        guard
            let paths = try? volume.children(
                of: Path(self),
                includingHidden: options == .includeHidden || options == .includeHiddenUnsorted,
            )
        else {
    ...
    @discardableResult
    public func setPermissions(_ permissions: Int, using volume: some Volume = FoundationVolume())
        throws -> Path {
        let path = Path(self)
