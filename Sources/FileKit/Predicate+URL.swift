...
extension InfraKit.Predicate where Value == URL {
    public static func fileExtensions(_ extensions: [String]) -> Self {
    ...
    public static var imageFiles: Self {
        utTypes([.png, .jpeg, .webP, .heic, .heif, .tiff, .gif, .bmp, .ico, .icns])
    }

    public static var videoFiles: Self {
        utTypes([.movie, .video, .mpeg4Movie, .quickTimeMovie, .avi])
    }

    public static var audioFiles: Self {
        utTypes([.audio, .mp3, .mpeg4Audio, .wav, .aiff])
    }

    public static var documentFiles: Self {
        utTypes([.pdf, .rtf, .plainText, .html, .xml, .yaml, .json])
    }

    public static func fileName(matching pattern: String) -> Self {
    ...
    public static func fileName(startingWith prefix: String) -> Self {
        fileName { $0.starts(with: prefix) }
    }

    public static func fileName(endingWith suffix: String) -> Self {
        fileName { $0.hasSuffix(suffix) }
    }

    public static func fileName(containing substring: String) -> Self {
        fileName { $0.contains(substring) }
    }

    public static func fileName(localizedStandardContaining substring: String) -> Self {
        fileName { $0.localizedStandardContains(substring) }
    }

    public static func fileName(localizedCaseInsensitiveContaining substring: String) -> Self {
    ...
    public static func fileSize(_ range: ClosedRange<Int>) -> Self {
        Self { url in
    ...
    public static func modifiedWithin(_ interval: TimeInterval) -> Self {
        Self { url in
            guard
    ...
    // why: nil means unknown I/O state, distinct from false.
