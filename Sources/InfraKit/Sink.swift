...
extension Sink {
    @preconcurrency
    public func filter(_ predicate: @escaping @Sendable (Element) -> Bool) -> some Sink<Element> {
        FilterSink(sink: self, predicate: predicate)
    }

    @preconcurrency
    public func map<T: Sendable>(_ transform: @escaping @Sendable (T) -> Element) -> some Sink<
        T,
    > {
        contramap(transform)
    }
    ...
    @preconcurrency
    public func contramap<T: Sendable>(_ transform: @escaping @Sendable (T) -> Element) -> some Sink<
        T,
    > {
