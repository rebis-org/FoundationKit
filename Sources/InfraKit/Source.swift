...
extension Source {
    @preconcurrency
    public func map<T: Sendable>(_ transform: @escaping @Sendable (Element) -> T) -> some Source<T> {
        MapSource(source: self, transform: transform)
    }

    @preconcurrency
    public func filter(_ predicate: @escaping @Sendable (Element) -> Bool) -> some Source<Element> {
        FilterSource(source: self, predicate: predicate)
    }

    @preconcurrency
    public func compactMap<T: Sendable>(_ transform: @escaping @Sendable (Element) -> T?)
        -> some Source<T> {
        CompactMapSource(source: self, transform: transform)
    }

    public func take(_ count: Int) -> some Source<Element> {
        TakeSource(source: self, count: count)
    }

    public func drop(_ count: Int) -> some Source<Element> {
        DropSource(source: self, count: count)
    }

    public func concat(_ other: some Source<Element>) -> some Source<Element> {
