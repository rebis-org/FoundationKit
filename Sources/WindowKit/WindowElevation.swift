    ...
    public protocol WindowElevation: Sendable {
        ...
        @MainActor
        func present(_ view: AnyView, on screen: NSScreen, at level: any WindowLevel) throws
            -> NSWindowController
    }
    ...
    public final class SkyLightWindowElevation: WindowElevation, @unchecked Sendable {
        private static let addFlags: Int32 = 7

        public static let shared: SkyLightWindowElevation = {
            do {
                return try unsafe SkyLightWindowElevation()
            } catch {
                let log = ErrLog<OSLogSink>(
                    identity: Identity(subsystem: "dev.rebis.WindowKit", category: "window"))
                log.log(error)
        ...
        // Main-actor serialization makes a lock unnecessary; every access below is @MainActor.
        private var spaces: [Int32: WindowSpace] = [:]
        ...
        public func elevate(_ window: NSWindow, to level: any WindowLevel) throws {
            let space = try space(for: level.rawValue)
        ...
        @preconcurrency
        @MainActor
        public func present(_ view: AnyView, on screen: NSScreen, at level: any WindowLevel) throws
            -> NSWindowController {
            let hosting = OverlayWindow.controller(screen: screen)
        ...
        @MainActor
        private func space(for level: Int32) throws -> WindowSpace {
            if let cached = spaces[level] {
                return cached
            }
