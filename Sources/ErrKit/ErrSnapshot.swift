...
extension ErrSnapshot {
    public var chainDescription: String {
        snapshots.enumerated().map { index, snapshot in let indent = String(repeating: "  ", count: index); let heading = "\(indent)\(snapshot.formattedType)"; guard snapshot.underlyingError != nil else { return "\(heading)\n\(indent)  \(snapshot.localizedDescription)" }; return heading }.joined(separator: "\n")
    }
