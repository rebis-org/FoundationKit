...
extension Item {
    ...
    public func appending(component: some StringProtocol) -> Path {
        Path(string: Path.join(string, component.split(separator: "/")))
    }

    public static func / (lhs: Self, rhs: some StringProtocol) -> Path {
        lhs.appending(component: rhs)
    }

    public func relative(to base: some Item) -> String {
        ...
        return
            (Array(repeating: "..", count: baseComponents.count - common)
                    + pathComponents.dropFirst(common)).joined(separator: "/")
    }
